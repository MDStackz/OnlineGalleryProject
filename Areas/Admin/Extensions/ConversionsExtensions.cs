...
namespace Memberships.Areas.Admin.Extensions
{
    ...
    public static class ConversionsExtensions
    {
        ...
        {
            var oldPi = await db.ProductItems.CountAsync(pi => pi.ProductId == productItem.OldProductId && pi.ItemId == productItem.OldItemId);

            var newPi = await db.ProductItems.CountAsync(pi => pi.ProductId == productItem.ProductId && pi.ItemId == productItem.ItemId);
